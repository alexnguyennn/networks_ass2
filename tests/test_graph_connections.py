...
def test_path_to_edge_tuples(graph, graph_topology, graph_topology_simple):
    ...
    assert (len(graph.path_list_to_edges(result_path))) == result_path - 1
    result_path = ['a', 'e', 'i']
    assert graph.path_list_to_edges(result_path) == [('a', 'e'), ('e', 'i')]
    assert (len(graph.path_list_to_edges(result_path))) == result_path - 1
    result_path = ['A', 'C', 'D']
    assert graph_topology_simple.path_list_to_edges(result_path) == [('A', 'C'), ('C', 'D')]
    assert (len(graph_topology_simple.path_list_to_edges(result_path))) == result_path - 1
    result_path = ['B', 'C']
    ...
    assert (len(graph_topology.path_list_to_edges(result_path))) == result_path - 1
...
def test_connections(graph):
    ...
    success = graph.add_connection(connection_edges)
    assert success == True
